...
@section('content')
    ...
    <div class="m-heading-1 border-green m-bordered">
        ...
        <p>This page is a list of export queues that have been created. You can download or delete files when the files have been generated by the system.</p>
    </div>
                    ...
                    <td>
                        <?php
	                        ...
	                        if(isset($filter['date_start']) && isset($filter['date_end'])){
	                            $html .= 'Date Start : '. date('d M Y', strtotime($filter['date_start'])).'<br>';
	                            $html .= 'Date End : '. date('d M Y', strtotime($filter['date_end'])).'<br>';
	                        }

	                        $deals_key = null;
	                        if( isset($filter['id_deals']) ){
	                        	$deals_key = array_search($filter['id_deals'], array_column($deals, 'id_deals'));
	                        }

	                        $deals_name = $deals[$deals_key]['deals_title'] ?? 'all';
	                        $html .= 'Deals : '. $deals_name.'<br>';

	                        $outlet_key = null;
	                        if( isset($filter['id_outlet']) ){
	                        	$outlet_key = array_search($filter['id_outlet'], array_column($outlet, 'id_outlet'));
	                        }

	                        $outlet_name = $outlet[$outlet_key]['outlet_name'] ?? 'all';
	                        $html .= 'Outlet : '. $outlet_name.'<br>';
